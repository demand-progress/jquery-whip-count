(function($) {
    $.fn.whipCount = function(o) {
        ...
        function positionDiv(position, percent) {
            return $("<div class='position " + position + "'>").css({
                "float": "left",
                width: percent + "%"
            });
        }
        return this.each(function() {
            ...
            $el.append($chart);
            if (opts.legend) {
                var legend = '<div class="wc-legend">' + '<p class="center">50%</p>' + '<p class="fili">60%</p>' + '<div class="firms">' + '  <div class="yea box pull-left"></div>' + '  <div class="label pull-left">YEA</div>' + '  <div class="nay box pull-right"></div>' + '  <div class="label pull-right">NAY</div>' + '  <div class="clear"></div>' + "</div>" + '<div class="leans">' + '  <div class="lean-yea box pull-left"></div>' + '  <div class="label pull-left">LEAN YEA</div>' + '  <div class="lean-nay box pull-right"></div>' + '  <div class="label pull-right">LEAN NAY</div>' + '  <div class="clear"></div>' + "</div>";
                $el.append(legend);
            }
        });
    };
